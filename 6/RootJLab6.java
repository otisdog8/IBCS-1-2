...
public class RootJLab6 {
    ...
    public static void main(String[] args) {
        int response;
        String[] items = { "End Program", "Horizontal Tabs", "Numbergame", "Rectangle", "Factorials",
                "Thousand Primes" }; // Menu code
    ...
    private static void numbergame() {
        ...
        do {
            ...
            if (guess % 1000 - guess % 100 == random % 1000 - random % 100) {
                numscorrect++;
            } // The only reason the 1000s number will ever be the same as the answer is when
              // the answer, or when the person for whatever reason decides to break the rules
            if (random == guess) {
                solved++;
    ...
    private static void rectangle() {
        ...
        for (int i = 0; i < height; i++) {
            for (int j = 0; j < width; j++) {
                if (j == 0 || j == width - 1 || i == 0 || i == height - 1) { // Tells it to print an asterix when it is
                                                                             // needed
                    System.out.print('*');
                } else {
                    System.out.print(' ');
    ...
    private static void factorials() {
        System.out.println("Enter the size of your factorial table"); // We're using longs here to stave off the
                                                                      // inevitable variable overflow
